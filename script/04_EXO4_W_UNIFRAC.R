#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
#
# EXAMPLE 4 : BETA DIVERSITY - WEIGHTED UNIFRAC -----
#
#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
abundance_table <- "exo4_16S.txt"
metadata_file <- "exo4_metadata_16S.txt"
...
# Show tree
ggtree(ps) + 
  geom_nodelab(aes(label=label), hjust=-.05, size=3.5)+ # tree scaffold
  geom_point(aes(x=x+hjust, color=Step, shape=Phylum, 
                 size=Abundance), na.rm=TRUE)+ # add the symbols wich size depends on abundance
  geom_tiplab(aes(label=Genus), hjust=-.35)+ # add the name of the OTUs
